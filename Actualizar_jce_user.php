...
$registro = mysqli_query($conexion, "UPDATE `usuarios` SET `id`=NULL,`nombre`='$_REQUEST[nombre]',`lugarDeNacimiento`='$_REQUEST[lugarDeNacimiento]',`fechaDeNacimiento`='$_REQUEST[fechaDeNacimiento]',`nacionalidad`='$_REQUEST[Nacionalidad]',`sexo`='$_REQUEST[sexo]',`sangre`='$_REQUEST[sangre]',`estadoCivil`='$_REQUEST[estadoCivil]',`ocupacion`='$_REQUEST[ocupacion]' WHERE nombre LIKE '$_REQUEST[nombre]'") or die("error en el update: " . mysqli_error($conexion));
    ...
    <body>
        <div class="row">
            <div class="col-5">
                ...
                <a href="Actualizar_jce.html" class="btn btn-outline-primary mx-2">
                Volver</a>
            </div>
        </div>
    </body>
